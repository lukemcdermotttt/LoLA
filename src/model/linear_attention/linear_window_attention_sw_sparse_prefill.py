...
def hybrid_attention_quadratic(q: torch.Tensor, k: torch.Tensor, 
    ...
    mask_window, mask_linear = get_masks(window_size, M, N, q.device) #NOTE: ADDED
    ...
    #print('sm-max scores')
    #print(torch.exp(a_sm - a_sm_max)[0,0,20:28,10:18])
    #print()
    ...
    if return_global_cache:
        if N > window_size:
            ...
            #torch.save(rest_f_k_f,'/home/archy2/luke/LoLA/checkpoints/f_k_f.pt')
            #print('saved')
        else:
            global_cache = (local_scores, None, None)
...
class LinearAttentionSparsePrefillSlidingWindowCache(LinearAttentionState):
    ...
    def update(self, key_states: torch.Tensor, value_states: torch.Tensor, 
        ...
        with torch.set_grad_enabled(grad_enabled):
            ...
            # KV state
            #NOTE: THIS DOESNT WORK, ONLY DECODE_KV_STATE WORKS.
        ...
        return None, None #self.kv_states[layer_idx], self.k_states[layer_idx]
...
6. update
- need to replace with finite memory attention instead of quadratic
"""