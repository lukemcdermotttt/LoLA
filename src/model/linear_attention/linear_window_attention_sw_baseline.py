...
def hybrid_attention_quadratic(q: torch.Tensor, k: torch.Tensor, 
    ...
    # 3. Combine
    a = ((a_sm) / (sum_sm)).to(q.dtype)  # Save attention weights
    # Allow outputs to also depend on prior kv_state and k_state
    y = torch.einsum('bhmn,bhnd->bhmd', a_sm, v.float())
...
# ---------------------
class LolcatsLinearSlidingWindowAttentionBaseline(LolcatsLinearAttention): #NOT ACTuALLY LINEAR, USING QUADRATIC
    """
        ...
        if self.train_attention:
            ...
            if past_key_value is None:  # Regular training
                ...
                if f_q.shape[2] == 1 and kv_seq_len > 1 and not self.training:  # Generating
                    ...
                    y_true = (torch.einsum('bhmn,bhnd->bhmd', a_sm, v_cache.float())) #+ linear_factors * torch.einsum('bhlf,bhfd->bhld', f_q.float(), f_kv_state.float()))
                    #sum_ln = linear_factors * torch.einsum(
                    #    'bhlf,bhnf->bhl', f_q.float(), f_k_state.float())[..., None]
                    #y_true = (y_true / (sum_sm + sum_ln)).to(q.dtype) 
                    y_true = (y_true / (sum_sm)).to(q.dtype) 
