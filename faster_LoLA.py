...
from src.model.linear_attention import LinearAttentionState, LolcatsLinearAttention
from flash_attn import flash_attn_func
from src.model.rotary import get_rotary_embeddings
...
if __name__ == "__main__":
    # Test configuration matching Llama model parameters
    B = 2  # batch size
    H = 32  # num heads (Llama uses 32)
    L = 1024  # sequence length
    D = 128  # head dimension (Llama uses 128)
    F = 32  # feature dimension for LoLA
    window_size = 128
    global_cache_size = 256

    # Create random input tensors matching Llama's expected shapes
    torch.manual_seed(42)
    hidden_states = torch.randn(B, L, H*D, dtype=torch.bfloat16, device='cuda')
    attention_mask = torch.ones(B, L, dtype=torch.bool, device='cuda')
    position_ids = torch.arange(L, dtype=torch.long, device='cuda').unsqueeze(0).expand(B, -1)

    # Create Llama config
    config = LlamaConfig(
        hidden_size=H*D,
        num_attention_heads=H,
        num_key_value_heads=H,  # For GQA, but we're using same number as attention heads
        ...
        pretraining_tp=1,
        torch_dtype=torch.bfloat16  # Add this to ensure model uses bfloat16
    )

    # Create base attention with layer_idx
    base_attn = LlamaAttention(config, layer_idx=0).to('cuda').to(torch.bfloat16)  # Convert to bfloat16
    
    # Create LoLA attention layer
    attention = LolcatsSparseSlidingWindowAttention(
        num_heads=H,
        head_dim=D,
        window_size=window_size,
        global_cache_size=global_cache_size,
        init_window_factor=0.5,
    ...
    ).to('cuda').to(torch.bfloat16)  # Convert to bfloat16

    # Test forward pass with Llama-like inputs
    print("\nTesting forward pass with Llama-like inputs...")
    
    # Warmup
    print("Warming up...")
    for _ in range(3):
        output, new_state, _ = attention(
    ...
    for i in range(num_runs):
        ...
        if i % 2 == 0:
            print(f"Run {i+1}/{num_runs}: {timings[-1]:.2f} ms")
    
    avg_time = sum(timings) / len(timings)
    print(f"\nAverage forward pass time: {avg_time:.2f} ms")
    print(f"Throughput: {1000/avg_time:.2f} sequences/second")
    print(f"Sequence length: {L}, Batch size: {B}, Hidden size: {H*D}")
    
    assert output.shape == (B, L, H*D), "Output shape mismatch!"

    # Test decoding (single token generation)
    #print("\nTesting decoding (single token generation)...")
    #single_token = hidden_states[:, :1, :]  # Take first token
    #output, new_state, _ = attention(
    #    hidden_states=single_token,
    #    attention_mask=attention_mask[:, :1],
    #    position_ids=position_ids[:, :1],
    #    past_key_value=new_state,
    #    use_cache=True
    #)
    #print(f"Output shape: {output.shape}")
    #print(f"Expected shape: [{B}, 1, {H*D}]")
    #assert output.shape == (B, 1, H*D), "Output shape mismatch!"
